...
def pre_voidy_calc(voids, cel_obj):
    ...
    for v_idx in voids.index:
        ...
        for grs_idx in s_idx:
           #print (cel_obj.loc[grs_idx, ['RAdeg', 'DEdeg', 'cmvd_Mpc']].values) # getting duplicate objects
            if len(cel_obj.loc[grs_idx].shape) != 1: #probably need to come back and fix this
                
                ra, de, s_cmvd = cel_obj.loc[grs_idx, ['RAdeg', 'DEdeg', 'cmvd_Mpc']].iloc[0].values
            else:
                ra, de, s_cmvd = cel_obj.loc[grs_idx, ['RAdeg', 'DEdeg', 'cmvd_Mpc']].values
...
def calc_master_voidiness(int_dict, cel_obj):
    ...
    for idx in list(cel_obj.index):
        if idx in int_dict.keys():
            ...
            void_sizes = np.mean(int_dict[idx]['void_size'])
            union = take_union(ints)
            cel_obj.at[idx, 'Voidiness'] = calc_voidiness(union)
            cel_obj.at[idx, 'Average Void Size'] = void_sizes
            cel_obj.at[idx, 'Max Void Size'] = np.max(int_dict[idx]['void_size'])

        else:
            cel_obj.at[idx, 'Voidiness']  = 0
            cel_obj.at[idx, 'Average Void Size'] = 0
            cel_obj.at[idx, 'Max Void Size'] = 0
